function punchAction(stats, info, embed) {
    ...
    embed.setDescription(`he deals ${other.name} ${dmg}<:HP:821850786870198293> of damage`)
    playerStats.punches++
...
function kickAction(stats, info, embed) {
    ...
    if (Math.random() <= fallRate) {
        ...
        embed.setTitle(`${player.name} is useless <:bang:822187290377584660>`)
        ...
        let dmg = randRange(...dmgs)
        embed.setTitle(`${player.name} kicks <:KickEmj:821827114906288178> ${other.name}`)
...
function shootPoisoned(stats, info, embed) {
    ...
    other.repeatedActs.push(poision.bind({
        poisonApplied,
        playerStats,
        otherStats
    }))
...
let types = {
    ...
    mage: {
        name: "mage",
        health: 100,
        pics: ["mage.jpg","Mage2.jpg","mage3.jpeg"],
        init: player => {
            player.mana = 7.5
        },
        actions: {
